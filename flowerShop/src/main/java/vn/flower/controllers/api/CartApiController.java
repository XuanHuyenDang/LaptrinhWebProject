...
public class CartApiController {
    ...
    // Endpoint mới để áp dụng mã giảm giá
    @PostMapping("/apply-discount")
    public ResponseEntity<CartView> applyDiscount(@RequestBody Map<String, String> payload) {
        String code = payload.get("code");
        if (code == null || code.trim().isEmpty()) {
            // Có thể trả về lỗi hoặc CartView hiện tại tùy logic
             throw new IllegalArgumentException("Mã giảm giá không được để trống.");
        }
        // currentAccountId() sẽ ném 401 nếu chưa đăng nhập
        CartView updatedCart = cartService.applyDiscountCode(currentAccountId(), code.trim());
        return ResponseEntity.ok(updatedCart);
    }

     // Endpoint mới để xóa mã giảm giá
    @PostMapping("/remove-discount") // Hoặc dùng @DeleteMapping nếu muốn
    public ResponseEntity<CartView> removeDiscount() {
        // currentAccountId() sẽ ném 401
         CartView updatedCart = cartService.removeDiscountCode(currentAccountId());
         return ResponseEntity.ok(updatedCart);
    }
